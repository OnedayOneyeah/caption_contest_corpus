...
def batch_to_device(batch, mode, args):
    image, choices, labels, raw_choices = batch['image'], batch['choices'], batch['label'], batch['raw_choices']
    if not args.use_accelerate or mode == 'val':
        image, choices, labels = map(
            lambda x: x.to(args.device),
            [image, choices, labels])

    return raw_choices, dict(zip(['image', 'choices', 'labels'], [image, choices, labels]))
        ...
        for mode in ['train', 'val']:
            ...
            for i, batch in bar:
                with torch.set_grad_enabled(mode=='train'):
                    raw_choices, batch = batch_to_device(batch, mode, args)
